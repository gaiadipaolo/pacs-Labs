...
class sparse_matrix : public std::vector<std::map<int,double>>{
    ...
    inline const size_t rows() const {return size();}; //equivalent to std::vector.size()

    inline const size_t cols() const {return m;};

    void csr(std::vector<double>& a, std::vector<int>& col_ind, std::vector<int>& row_ptr);

    void operator += (sparse_matrix& rhs); // a+=a;

    // b = A*x;
    friend std::vector<double> operator*(sparse_matrix& M, const std::vector<double>& x);
